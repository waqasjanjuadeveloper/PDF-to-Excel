...
def extract_ledger_page(page):
    ...
    date_indices = [i for i, (_, line) in enumerate(lines) if line and DATE_RE.fullmatch(line[0]["text"].strip())]
    records = []
    start_indices = []
    for date_idx in date_indices:
        ...
        if date_idx > 0:
            ...
            pre_number_words = [w["text"].strip() for w in prev_line if 85 <= float(w["x0"]) < 155]
    ...
    for pos, date_idx in enumerate(date_indices):
        ...
        if not transaction_lines: continue
        date = lines[date_idx][1][0]["text"].strip()
        type_value, number, customer_parts, ref_date_words = "", "", [], []
        amount_words = {"debit": [], "credit": [], "pdc": [], "balance": []}
        for _, line in transaction_lines:
            for word in line:
                text = clean_cell(word["text"])
                x0 = float(word["x0"])
                if 55 <= x0 < 85 and not type_value: type_value = text
                elif 85 <= x0 < 155 and not number: number = text
                elif 155 <= x0 < 323: customer_parts.append(text)
                elif 323 <= x0 < 410:
                    if DATE_RE.search(text): ref_date_words.append(text)
                elif 410 <= x0 < 490: amount_words["debit"].append(text)
                elif 490 <= x0 < 570: amount_words["credit"].append(text)
                elif 570 <= x0 < 630: amount_words["pdc"].append(text)
                elif x0 >= 630: amount_words["balance"].append(text)
        ...
        records.append({
            "Date": date, "Type": type_value, "Number": number,
            "Customer Reference": clean_cell(" ".join(customer_parts)),
            "Ref. Date": ref_date, "Debit": first_money(amount_words["debit"]),
            "Credit": first_money(amount_words["credit"]), "PDC Value": first_money(amount_words["pdc"]),
            "Balance": balance, "Due Date": due_date, "Paid Date": paid_date,
        })
    return records
...
def extract_tables(pdf_path):
    ...
    with pdfplumber.open(pdf_path) as pdf:
        for page_no, page in enumerate(pdf.pages, start=1):
            try:
                page_tables = page.extract_tables({"vertical_strategy": "lines", "horizontal_strategy": "lines", "snap_tolerance": 3, "join_tolerance": 3, "intersection_tolerance": 3})
            except Exception: page_tables = []
            for table_no, table in enumerate(page_tables or [], start=1):
                cleaned = [[clean_cell(v) for v in (row or [])] for row in table]
                cleaned = [r for r in cleaned if any(r)]
                if cleaned:
                    width = max(len(r) for r in cleaned)
                    tables.append({"page": page_no, "table": table_no, "data": [r + [""] * (width-len(r)) for r in cleaned]})
    return tables
...
def make_excel(job_dir, records, page_records, total_pages):
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        if records:
            pd.DataFrame(records, columns=LEDGER_COLUMNS).to_excel(writer, sheet_name="All Data", index=False)
            for page_no, rows in page_records.items():
                pd.DataFrame(rows, columns=LEDGER_COLUMNS).to_excel(writer, sheet_name=f"Page {page_no}"[:31], index=False)
        else:
            tables = extract_tables(job_dir / "source.pdf") if (job_dir / "source.pdf").exists() else []
            if tables:
                all_rows = [[item["page"], item["table"]] + row for item in tables for row in item["data"]]
                ...
                pd.DataFrame([["No extractable data found"]]).to_excel(writer, sheet_name="Result", index=False, header=False)
...
def convert():
    ...
    if not file or not file.filename: return jsonify(error="No PDF file uploaded."), 400
    if Path(file.filename).suffix.lower() not in ALLOWED: return jsonify(error="Only PDF files are allowed."), 400
    ...
    try:
        file.save(pdf_path)
        with pdfplumber.open(pdf_path) as pdf: total_pages = len(pdf.pages)
        ...
        return jsonify(job_id=job_id, filename="converted.xlsx", pages=total_pages, rows=len(records), sheets=(1 + ledger_pages) if records else 1, excel_data=excel_data)
